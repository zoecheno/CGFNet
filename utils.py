...
def confusion_matrix(outputs, labels, conf_matrix):
...
def plot_confusion_matrix(classes, cm, savename, title='Confusion Matrix', normalize=False):
    if normalize:
    ...
    #     c = cm[y_val][x_val]
    #     if c > 0.001:
    #         plt.text(x_val, y_val, "%0.0f" % (c,), color='red', fontsize=8, va='center', ha='center')
...
def accuracy(output, target, lam=1.0, topk=(1,), is_mixup=False):
    ...
    with torch.no_grad():
        ...
        if is_mixup:
            ...
            correct_a = pred.eq(target[0].data.view(1, -1).expand_as(pred))
            correct_b = pred.eq(target[1].data.view(1, -1).expand_as(pred))
            for k in topk:
                correct_k = lam * correct_a[:k].contiguous().view(-1).float().cpu().sum(0, keepdim=True) + (1 - lam) * \
                            correct_b[:k].view(-1).float().cpu().sum(0, keepdim=True)
